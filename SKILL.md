...
`coverage_strategy` 描述采用什么验证方式和覆盖内容，是从 `verification_goal` 到实际 coverage 实现的最小映射，不按 TP category 机械套用 testcase、covergroup、assertion 模板；该规则不得取消由 verification intent 自然决定的 coverage method。
